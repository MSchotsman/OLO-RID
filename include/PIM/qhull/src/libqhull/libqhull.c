...
/* The MathWorks, Inc. added the following two functions to manage error messages */
int mwprintf( FILE *stream, const char *format, ... ) {
  ...
  lcurr = strlen(qh mwerrmsg);
  if (lcurr < qh_buflen-1) { /* Only append if buffer is not currently full */
    va_start( marker, format );  
    lnew = vsprintf( s, format, marker );
    if (lnew > 0) {
      strncat( qh mwerrmsg, s, qh_buflen-lcurr-1 );
    }
    va_end( marker ); 
  }
