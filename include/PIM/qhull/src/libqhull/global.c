...
/*<html><pre>  -<a                             href="qh-globa.htm"
  >-------------------------------</a><a name="TOP">-</a>
   ...
   $Date: 12/17/2019 $
   $Author: Siju Thomas $
   $Summary: Prevent crash for higher dimensions by trapping the overflow and casting int to size_t $
 */
...
void qh_freebuffers(void) {
  ...
  /* MathWorks Changes 12-17-2019 - Siju Thomas */
  /* Begin Change */
  qh_memfree(qh NEARzero, (size_t)qh hull_dim * sizeof(realT));
  qh_memfree(qh lower_threshold, (size_t)(qh input_dim+1) * sizeof(realT));
  qh_memfree(qh upper_threshold, (size_t)(qh input_dim+1) * sizeof(realT));
  qh_memfree(qh lower_bound, (size_t)(qh input_dim+1) * sizeof(realT));
  qh_memfree(qh upper_bound, (size_t)(qh input_dim+1) * sizeof(realT));
  qh_memfree(qh gm_matrix, (size_t)(qh hull_dim+1) * qh hull_dim * sizeof(coordT));
  qh_memfree(qh gm_row, (size_t)(qh hull_dim+1) * sizeof(coordT *));
  /* End Change */
...
void qh_initqhull_buffers(void) {
  ...
  /* MathWorks Changes 12-17-2019 - Siju Thomas */
  /* Begin Change */
  qh NEARzero= (realT *)qh_memalloc((size_t)(qh hull_dim) * sizeof(realT));
  qh lower_threshold= (realT *)qh_memalloc((size_t)(qh input_dim+1) * sizeof(realT));
  qh upper_threshold= (realT *)qh_memalloc((size_t)(qh input_dim+1) * sizeof(realT));
  qh lower_bound= (realT *)qh_memalloc((size_t)(qh input_dim+1) * sizeof(realT));
  qh upper_bound= (realT *)qh_memalloc((size_t)(qh input_dim+1) * sizeof(realT));
  /* End Change */
  for (k=qh input_dim+1; k--; ) {  /* duplicated in qh_initqhull_buffers and qh_clear_ouputflags */
    qh lower_threshold[k]= -REALmax;
    qh upper_threshold[k]= REALmax;
  ...
  /* MathWorks Changes 12-17-2019 - Siju Thomas */
  /* Begin Change */
  qh gm_matrix= (coordT *)qh_memalloc((size_t)(qh hull_dim+1) * (size_t)(qh hull_dim) * sizeof(coordT));
  qh gm_row= (coordT **)qh_memalloc((size_t)(qh hull_dim+1) * sizeof(coordT *));
  /* End Change */
} /* initqhull_buffers */
...
/* --------------The MathWorks, Inc. added this function -------- */
void qh_init_mwerrmsg (void) { qh mwerrmsg[0] = '\0'; }
